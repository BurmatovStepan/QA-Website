...
- `npm run build:dev` - копирует `assets/` и собирает `scss/style.scss` и `ts/main.ts` в `dist/static/` без оптимизаций.
- `npm run build` - собирает проект в `dist/static/` со сжатием `style.css`.
...
> [!WARNING]
> Команды `npm start`, `npm run build:dev` и `npm run build` удаляют директорию `dist/static/` перед выполнением.
